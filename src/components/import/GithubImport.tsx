...
import { Github, ChevronLeft, ArrowLeftRight } from 'lucide-react';
...
export default function GithubImport() {
  ...
  return (
    <div className={cn('w-full max-w-4xl mx-auto px-6 py-10')}>
      ...
      {/* Heading */}
      <h1 className={cn('text-3xl md:text-4xl font-extrabold', isDarkMode ? 'text-darkmode-primarytxt' : 'text-lightmode-primarytxt')}>
        {isRTL ? 'الاستيراد من GitHub إلى Banild' : 'Import from GitHub to Banild'}
      </h1>
      <p className={cn('mt-2 mb-6', isDarkMode ? 'text-darkmode-secondarytxt' : 'text-lightmode-secondarytxt')}>
        {isRTL ? 'استورد مشروعك من GitHub وابدأ العمل في Banild' : 'Import your project from GitHub and start building in Banild'}
      </p>

      {/* Card */}
      <div className={cn('rounded-2xl border-1', isDarkMode ? 'bg-darkmode-secondary border-darkmode-primary' : 'bg-lightmode-primary border-lightmode-tertiary')}>
        <div className="p-5 md:p-6">
          ...
          {/* privacy options removed */}

          {/* final action moved above */}
        </div>
      </div>
    </div>
  );
